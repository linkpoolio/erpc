  ...
  - id: main
    ...
    upstreams:
      ...
      - endpoint: https://arbitrum-one.blastapi.io/xxxxxxx-xxxxxx-xxxxxxx
        ...
        # (OPTIONAL) NodeGroup marks upstreams that target the same physical backend via
        # different transports (e.g. HTTP + WebSocket). Members of the same nodeGroup
        # share block-head observations: when a WS upstream receives a newHeads
        # notification, sibling upstreams with the same nodeGroup get their state
        # poller updated too. This avoids stale-state ErrUpstreamBlockUnavailable
        # rejections when requests are routed (or hedged) to the HTTP sibling on
        # fast-block chains. Leave unset unless you run paired HTTP/WS upstreams
