...
// SuggestLatestBlock on *-ws-* must also advance the HTTP twin poller so
// EvmLeaderUpstream / partition prefer the same physical node.
func TestNetworkHandle_SuggestLatestBlock_BumpsHttpTwin(t *testing.T) {
	util.ResetGock()
	defer util.ResetGock()
	util.SetupMocksForEvmStatePoller()
	...
	cfgs := []*common.UpstreamConfig{
		{Type: common.UpstreamTypeEvm, Id: "internal-eth-mainnet-reth-ws-0", Endpoint: "http://rpc-ws.localhost", Evm: &common.EvmUpstreamConfig{ChainId: 123}},
		{Type: common.UpstreamTypeEvm, Id: "internal-eth-mainnet-reth-0", Endpoint: "http://rpc-0.localhost", Evm: &common.EvmUpstreamConfig{ChainId: 123}},
		{Type: common.UpstreamTypeEvm, Id: "internal-eth-mainnet-reth-1", Endpoint: "http://rpc-1.localhost", Evm: &common.EvmUpstreamConfig{ChainId: 123}},
	}
	for _, host := range []string{"rpc-ws.localhost", "rpc-0.localhost", "rpc-1.localhost"} {
		gock.New("http://" + host).Post("").Persist().
	...
	upstreamsRegistry := upstream.NewUpstreamsRegistry(
		ctx, &log.Logger, "test", cfgs, ssr, rateLimitersRegistry, vr, pr, nil, metricsTracker, nil,
	)
	...
	for _, u := range upstreamsRegistry.GetNetworkUpstreams(ctx, util.EvmNetworkId(123)) {
		byID[u.Id()] = u
		u.EvmStatePoller().SuggestLatestBlock(1000)
	}

	(&networkHandle{nw: network}).SuggestLatestBlock("ws:internal-eth-mainnet-reth-ws-0", 1001, nil)
